...
include_once("libs/f_seguridad.php");
...
if(!empty($_REQUEST['txtUsuario']) and (!empty($_REQUEST['txtClave'])))
{
		...
		if ($d_acceso[0] == 'S')
		{
			...
			header("location:softwork/pghome.php");	
			/*
		...
		{
			f_sessionclose();
			header("location:acceso.php?inderror=2");
			exit;
		}	
		elseif ($d_acceso[0] == 'N')
		{
			f_sessionclose();
			header("location:acceso.php?inderror=3");
			exit;
		}	
		elseif ($d_acceso[0] == 'B')
		{
			f_sessionclose();
			header("location:acceso.php?inderror=4");
			exit;
		}	
}
else
{	
	f_sessionclose();
	header("location:acceso.php?inderror=1");
	exit;
}
